...
1. Чем отличаются клиентские и серверные сокеты?
    серверные сокеты слушают (получают информацию),
    клиентские подключаются и посылают запросы
2. Как можно передавать через сокеты текстовую информацию?
    путём отправления байтов
3. Какие операции с сокетами блокируют выполнение программы?
    ожидание получения и отправки данных, подключения
4. Что такое неблокирующие сокеты?
    в случае неблокирующего сокета эти операции возвращают сразу,
    даже если ещё не были завершены
5. В чем преимущества и недостатки использования TCP по сравнению с UDP?
    TCP гарантирует передачу данных, но менее производительный
